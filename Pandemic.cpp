...
#pragma region Set
	...
	// uncomment the below to test cure
	/* for (int i = 0; i < 5; i++)                   
	{
		players[0].hand[i] = &PlayerDeck[i];
		players[0].hand[i]->status = 1;
		players[1].hand[i] = &PlayerDeck[i+12];
		players[1].hand[i]->status = 1;
		players[2].hand[i] = &PlayerDeck[i+24];
		players[2].hand[i]->status = 1;
		players[3].hand[i] = &PlayerDeck[i+36];
		players[3].hand[i]->status = 1;
	}
	*/
	choice = 0;
