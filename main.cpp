...
static const int MaxInfectionCards = 48;	//one for each city
static const int MaxPlayerCards = 59;		//48 city, 6 epidemic, 5 special event
...
#pragma endregion Up
	...
	while(end == false)
	{
		...
		while( players[p].actions > 0 )
		{
			...
			for( int i = 0; i < 7; i++ )
			{
				if( players[p].hand[i] != NULL && (players[p].hand[i]->color > 0 && players[p].hand[i]->color < 5) && players[p].hand[i]->status !=2 )
					show = true;
			}
			if( show == true )
			{
			...
			for( int i = 0; i < 7; i++ )
			{
				if( players[p].hand[i] && players[p].hand[i]->name == players[p].currentCity->name && players[p].hand[i]->status !=2)
					show = true;
			}
			if( show == true )
			{
